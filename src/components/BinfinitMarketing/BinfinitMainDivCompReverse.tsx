...
interface dataProps {
  data: reverseDataProps;
}
const BinfinitMainDivCompReversed: React.FC<dataProps> = ({ data }) => {
  const { title, desc, img, viewtxt, viewmoreredirect } = data;
  return (
    <div className="w-full h-auto md:h-[80vh] shadow-sm flex flex-col md:flex-row-reverse">
      {/* Image Section */}
      <div className="md:w-[58.33%] w-full overflow-hidden h-full md:h-full group">
        <Image
          ...
          height={1000}
          alt="mineral"
          className="object-cover object-center w-full h-full transition-transform duration-500 ease-out transform origin-center group-hover:scale-110"
        />
      </div>

      {/* Content Section */}
      <div className="bg-bgNewCompMain md:w-[41.66%] w-full flex justify-center items-start md:px-20 px-8 py-6 md:py-0 flex-col">
        <div>
          <h3 className="text-2xl md:text-3xl font-semibold mb-3 text-black">
            {title || "Marketing"}
          </h3>

          <p className="text-base md:text-lg text-black/[0.8] mb-4">
            {desc ||
...
export default BinfinitMainDivCompReversed;
